...
def parse_request(request):
    """Returns the URI /sample.txt from ['GET', '/sample.txt', 'HTTP/1.1']"""
...
def response_method_not_allowed():
    ...
    return b"\r\n".join([b"HTTP/1.1 405 Method Not Allowed", b"", b"You can't do that on this server!",])
...
def response_not_found():
    ...
    # Allowed response.
    return b"\r\n".join([b"HTTP/1.1 404 Method Not Found", b"", b"You can't do that on this server!",])
...
def resolve_uri(uri):
    ...
    # uri = /sample.txt or /sample_1.png
    if uri == '/':
        # webroot directory = 'C:\\workspace\\socket-http-server-homework/webroot'
        content = '\n'.join([row for row in os.listdir('/'.join([os.getcwd(), 'webroot']))]).encode()
        mime_type = 'text/plain'
    # If the URI is a file, it should return the contents of the file and its correct mimestype
    else:
        try:
            # open file 'C:\\workspace\\socket-http-server-homework/webroot/images/sample_1.png'
            with open(''.join(['/'.join([os.getcwd(), 'webroot']), uri]), 'rb') as f:
                content = f.read()
